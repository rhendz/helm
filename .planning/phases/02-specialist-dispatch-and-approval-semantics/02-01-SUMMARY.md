...
tags: [workflow, postgres, sqlalchemy, pydantic, worker, approvals]
requires:
  - phase: 01-durable-workflow-foundation
    provides: durable workflow runs, steps, artifacts, and validation blocking semantics
provides:
...
affects: [phase-02-approval-semantics, phase-03-adapter-writes, operator-inspection]
tech-stack:
  added: []
  patterns: [kernel-owned specialist execution, semantic step registry, durable invocation lineage]
key-files:
  created:
    - migrations/versions/20260313_0008_specialist_dispatch.py
    - packages/storage/src/helm_storage/repositories/workflow_specialist_invocations.py
  modified:
...
patterns-established:
  - "Specialist dispatch keys are (workflow_type, step_name) tuples owned by orchestration, not worker-local step-name maps."
  - "Specialist invocation input references always point at persisted upstream artifacts, and output references point at persisted downstream artifacts."
requirements-completed: [AGNT-01, AGNT-02, AGNT-03, ARTF-04, DEMO-02, DEMO-03]
duration: 6 min
