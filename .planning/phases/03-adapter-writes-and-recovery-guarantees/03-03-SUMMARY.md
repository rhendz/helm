...
plan: 03
subsystem: database
tags: [postgres, sqlalchemy, alembic, workflow-recovery, replay-lineage]
requires:
  - phase: 03-adapter-writes-and-recovery-guarantees
    provides: idempotent sync records, reconciliation-first retry, restart-safe sync execution
provides:
...
affects: [workflow-status-projection, api, telegram-bot, worker-replay]
tech-stack:
  added: [alembic migration 20260313_0012]
  patterns: [storage-first recovery facts, append-only replay lineage, terminal partial-sync snapshots]
key-files:
  created: [.planning/phases/03-adapter-writes-and-recovery-guarantees/03-03-SUMMARY.md, migrations/versions/20260313_0012_workflow_recovery_lineage.py]
  modified: [packages/storage/src/helm_storage/models.py, packages/storage/src/helm_storage/repositories/contracts.py, packages/storage/src/helm_storage/repositories/workflow_sync_records.py, packages/storage/src/helm_storage/repositories/replay_queue.py, packages/orchestration/src/helm_orchestration/schemas.py, packages/orchestration/src/helm_orchestration/workflow_service.py, tests/unit/test_workflow_orchestration_service.py, tests/unit/test_workflow_repositories.py]
key-decisions:
...
patterns-established:
  - "Recovery facts are persisted on sync rows and echoed as workflow events whenever retry, replay, or terminal transitions occur."
  - "Same-run retry reuses lineage generation 0 rows, while explicit replay appends a new lineage generation."
requirements-completed: [RCVR-03]
duration: 10 min
