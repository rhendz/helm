...
# A fixed "Sunday evening" reference used to freeze time in scheduling tests.
# When now = Sunday 2099-01-05 20:00 UTC, compute_reference_week returns
# Mon 2099-01-06 00:00 UTC, and all Mon–Fri 9am+ slots are in the future.
_FUTURE_MONDAY_MIDNIGHT = datetime(2099, 1, 5, 0, 1, 0)  # 2099-01-05 is a Monday
...
def test_weekly_scheduling_worker_flow_uses_shared_request_contract() -> None:
    # Freeze "now" to a future Monday morning so past_event_guard never fires for
    # same-week slots. Without this, slots generated by compute_reference_week for
    # the current real-world week may be in the past when tests run mid-week.
    _future_now = _FUTURE_MONDAY_MIDNIGHT.replace(tzinfo=UTC)
    with patch("helm_orchestration.scheduling.datetime") as mock_dt:
