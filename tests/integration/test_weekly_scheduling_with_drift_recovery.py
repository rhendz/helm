...
- S01: Real GoogleCalendarProvider writes and reads events correctly
...
# Freeze scheduling "now" to a future Monday at 00:01 UTC so all Mon-Fri slots
# generated by compute_reference_week are in the future, preventing past_event_guard
# from moving tasks into carry_forward during test runs mid-week.

_FUTURE_MONDAY_MIDNIGHT = _real_datetime(2099, 1, 5, 0, 1, 0, tzinfo=UTC)


@contextmanager
def _freeze_scheduling_time():
    """Context manager: freeze helm_orchestration.scheduling.datetime.now to a future Monday."""
    with _mock_patch("helm_orchestration.scheduling.datetime") as mock_dt:
        mock_dt.now.return_value = _FUTURE_MONDAY_MIDNIGHT
...
def test_telegram_sync_visibility(monkeypatch) -> None:
    ...
    for client, session in _client():
        ...
        # Step 2: Advance to approval and approve (time-frozen so past_event_guard doesn't fire)
        with _mock_patch("helm_orchestration.scheduling.datetime") as _mock_dt:
            _mock_dt.now.return_value = _FUTURE_MONDAY_MIDNIGHT
