...
async def handle(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not update.message:
        return
    threads = _service.list_waiting_on_other_party_threads()
    if not threads:
        await update.message.reply_text("No waiting-on-other-party threads.")
        return
    lines = ["Waiting-on-other-party threads:"]
