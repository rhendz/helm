...
def get_email_threads(limit: int = Query(default=20, ge=1, le=100)) -> list[EmailThreadResponse]:
    return [EmailThreadResponse(**item) for item in list_email_threads(limit=limit)]
...
def get_email_proposals(
    ...
    return [EmailProposalResponse(**item) for item in list_email_proposals(limit=limit)]
...
def get_email_drafts(limit: int = Query(default=20, ge=1, le=100)) -> list[EmailDraftResponse]:
    return [EmailDraftResponse(**item) for item in list_email_drafts(limit=limit)]
